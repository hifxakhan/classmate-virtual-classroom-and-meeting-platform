...
**Symptoms:** Transcription returns empty, "Model load failed"

**Solutions:**
```powershell
# 1. Check health endpoint
Invoke-WebRequest "https://backend.railway.app/api/whisper-health"

# 2. Check Railway logs for model loading
railway logs --follow | Select-String "Whisper"

# 3. Verify ffmpeg is installed (Docker image includes it)
# Open Railway deploy logs during first request

# 4. Whisper model download may take 5-10 mins on first use
# Allow 15 seconds for first transcription request
```
...
**Railway Dashboard:**
- Memory usage (Whisper model uses ~200MB)
- CPU usage
- Network I/O
- Database connections

**Recommended limits:**
- Memory: 512MB → 1GB with Whisper
...
| `WHISPER_MODEL_SIZE` | Whisper AI model | `tiny` |
